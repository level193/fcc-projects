...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams LEFT JOIN games AS w ON teams.team_id = w.winner_id LEFT JOIN games AS o ON teams.team_id = o.opponent_id WHERE (w.round = 'Eighth-Final' OR o.round = 'Eighth-Final') AND (w.year = '2014' OR o.year = '2014')")"
